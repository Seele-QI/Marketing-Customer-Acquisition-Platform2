...
| C. 业务工作流 | `RUNNINGHUB_API_KEY`（国内视频/音频）、`RUNNINGHUB_IMAGE_API_KEY` + `RUNNINGHUB_IMAGE_BASE_URL`（海外图片）、RH App/Workflow ID | Key 与图片基址可 sync；Workflow ID 按功能配置 | Key：是；Workflow：宜后台按功能 | Key 扁平原或 RH 专区；ID 勿丢进 LLM 池 |
