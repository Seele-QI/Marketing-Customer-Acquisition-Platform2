...
from lib.publisher.connector_base import BaseConnector, PublishResult
...
class DouyinConnector(BaseConnector):
    ...
    UPLOAD_URLS = (
        ...
        "https://creator.douyin.com/creator/content/upload",
    )
    
    @staticmethod
    async def _goto_upload_page(page, *, timeout_ms: int = 90000) -> None:
        """抖音上传页资源多，不能用 wait_until=load，否则 30s 必超时。"""
        last_err: Exception | None = None
        for url in DouyinConnector.UPLOAD_URLS:
            for attempt in range(2):
                try:
                    print(f"[Douyin] 导航到: {url} (第 {attempt + 1} 次)")
    ...
    async def _publish_video(self, video_path: str, desc: str, title: str, options: Dict[str, Any] = None, **kwargs) -> PublishResult:
        ...
        search_paths = [
            video_path,
            os.path.join(root_dir, "public", "video-cache", "video-cache", "manual-uploads", filename),
            os.path.join(root_dir, "data", "temp", filename),
            os.path.join(root_dir, "public", "video-cache", "video-postprocess", filename),
        ]
        ...
        if final_video_path:
            video_path = final_video_path
             ...
             video_path = os.path.join(root_dir, "data", "temp", filename)
        ...
        try:
            ...
            tmp_dir = os.path.join(root_dir, "data", "temp")
            os.makedirs(tmp_dir, exist_ok=True)
            print(f"[Douyin] 使用本地临时目录: {tmp_dir}")

            browser_path = prepare_playwright_browsers_path(root_dir)
            
            print(f"[Douyin] Root Dir: {root_dir}")
            print(f"[Douyin] Browser Path: {browser_path or 'playwright default'}")
            ...
            async with async_playwright() as p:
                ...
                cookies = []
                for key, value in self.credentials.items():
                    if key == "cookies" or value is None or isinstance(value, (list, dict)):
                        continue
                    cookies.append({
                        "name": key,
                        "value": str(value),
                ...
                cookie_path = os.path.join(root_dir, "data", "profiles", "douyin_cookies.json")
                ...
                await self._goto_upload_page(page)
                try:
                    await page.wait_for_load_state("networkidle", timeout=10000)
                except Exception:
                    print("[Douyin] networkidle 超时，继续尝试后续操作")
                await asyncio.sleep(2)
                ...
                if "login" in page.url or "passport" in page.url or await page.locator('text="扫码登录"').is_visible() or await page.locator('text="验证码登录"').is_visible():
                     ...
                     if not password_login_success:
                         ...
                         if await qr_code.count() > 0:
                             ...
                             qr_path = os.path.join(self.get_project_root(), "data", "temp", "douyin_login_qr.png")
                             ...
                             try:
                                 ...
                                 if sys.platform == "win32":
                                     os.startfile(display_path)
                                 elif sys.platform == "darwin":
                                     subprocess.run(["open", "-a", "Preview", display_path], check=False)
                                 else:
                                     subprocess.run(["xdg-open", display_path], check=False)
                                 print(f"[Douyin] 已尝试打开二维码: {display_path}")
                             except Exception:
                                 pass
                             ...
                             if login_success:
                                 ...
                                 try:
                                     ...
                                     cookie_path = os.path.join(self.get_project_root(), "data", "profiles", "douyin_cookies.json")
                ...
                def extract_video_cover(v_path, c_path):
                    ...
                    local_ff = os.path.join(root_dir, ".browsers", "ffmpeg-1011", "ffmpeg.exe")
                    ...
                    try:
                        if os.path.exists(c_path):
                            os.remove(c_path)
                        subprocess.run(
                            [ffmpeg_path, "-y", "-ss", "00:00:01", "-i", v_path, "-frames:v", "1", "-q:v", "2", c_path],
