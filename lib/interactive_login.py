...
LOGIN_SESSION_DIR = TEMP_ROOT / "login-sessions"
...
class InteractiveBrowserLogin:
    ...
    def _session_record_path(self, session_id: str) -> Path:
        ...
        return LOGIN_SESSION_DIR / f"{safe_id}.json"

    def _persist_session_record(self, session_id: str, session: Dict[str, Any]) -> None:
        endpoint = str(session.get("cdp_endpoint") or "")
        ...
        record = {
            ...
            "start_time": float(session.get("start_time") or time.time()),
        }
        target = self._session_record_path(session_id)
        temporary = target.with_suffix(".tmp")
        temporary.write_text(json.dumps(record, ensure_ascii=False), encoding="utf-8")
        temporary.replace(target)

    def _remove_session_record(self, session_id: str) -> None:
        try:
            self._session_record_path(session_id).unlink(missing_ok=True)
        except Exception:
            logger.exception("remove login session record failed session=%s", session_id)

    async def _recover_persisted_session(
        self,
        session_id: str,
        *,
        user_id: int,
    ) -> dict | None:
        record_path = self._session_record_path(session_id)
        ...
        try:
            ...
            if start_time <= 0 or time.time() - start_time > 600:
                self._remove_session_record(session_id)
                return None
            endpoint = str(record.get("cdp_endpoint") or "")
            if not endpoint.startswith("http://127.0.0.1:"):
                self._remove_session_record(session_id)
                return None
            platform = storage_platform(str(record.get("platform") or ""))
            if platform not in self.platform_config and platform in EXTRA_PLATFORM_CONFIG:
                self.platform_config[platform] = EXTRA_PLATFORM_CONFIG[platform]
            ...
            browser = await asyncio.wait_for(
                self.playwright.chromium.connect_over_cdp(endpoint),
                timeout=8,
            )
            ...
            logger.info("[%s] recovered login session=%s endpoint=%s", platform, session_id, endpoint)
            return session
        except PermissionError:
            raise
        except Exception:
            logger.exception("recover login session failed session=%s", session_id)
            self._remove_session_record(session_id)
            return None

    async def _recover_latest_platform_session(
        self,
        *,
        user_id: int,
        platform: str,
    ) -> None:
        if not LOGIN_SESSION_DIR.exists():
            return
        records = sorted(
            LOGIN_SESSION_DIR.glob("*.json"),
        ...
        for record_path in records:
            try:
                record = json.loads(record_path.read_text(encoding="utf-8"))
                if (
                    int(record.get("user_id")) == int(user_id)
                    and storage_platform(str(record.get("platform") or "")) == storage_platform(platform)
                ):
                    await self._recover_persisted_session(record_path.stem, user_id=user_id)
                    return
            except Exception:
                continue
    ...
    async def _launch_native_cdp(self, executable: str, user_data_dir: Path, login_url: str):
        ...
        return process, browser, context, page, endpoint
    ...
    async def start_interactive_login(self, platform: str, *, user_id: int) -> Dict[str, Any]:
        ...
        if not self._find_active_session(user_id, platform):
            await self._recover_latest_platform_session(user_id=user_id, platform=platform)
        ...
        try:
            ...
            if browser_launch_mode(key) == "native_cdp":
                ...
                for candidate in sorted(candidates, key=lambda item: item["source"] != "system_chrome"):
                    try:
                        ...
                        native_process, browser, context, page, cdp_endpoint = await self._launch_native_cdp(
            ...
            self.active_sessions[session_id] = session
            self._persist_session_record(session_id, session)
    ...
    async def check_login_status(self, session_id: str, *, user_id: int | None = None) -> Dict[str, Any]:
        ...
        if session_id not in self.active_sessions:
            try:
                await self._recover_persisted_session(session_id, user_id=int(user_id or 0))
            except PermissionError:
                return {"status": "error", "code": "session_not_owned", "error": "登录会话不属于当前用户"}
        if session_id not in self.active_sessions:
