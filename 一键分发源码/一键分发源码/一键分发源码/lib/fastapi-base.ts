...
const DEV_FASTAPI_DEFAULT = "http://127.0.0.1:8002"

function sanitizeProxyHeaders(headers: Headers) {
  headers.delete("host")
  headers.delete("connection")
  headers.delete("content-length")
  // Node fetch 不支持转发 expect，会导致 POST 代理失败（登录/注册 500）
...
export async function proxyMultipartToFastapi(req: Request, path: string): Promise<Response> {
  ...
  const headers = new Headers(req.headers)
  sanitizeProxyHeaders(headers)
  // 重建 FormData 后必须去掉原 Content-Type，否则 boundary 不匹配导致 FastAPI 解析失败
