...
import { dispatchAuthChanged, setCachedUserId } from "@/lib/auth-session"
import { clearShareVideos } from "@/lib/video/storage"
...
export function UserMenu() {
  ...
  const submitAuth = async () => {
    ...
    try {
      ...
      const nextMe = (data as Me) ?? null
      setMe(nextMe)
      setCachedUserId(nextMe?.user?.id ?? null)
      dispatchAuthChanged({ loggedIn: true, userId: nextMe?.user?.id ?? null })
      setOpenLogin(false)
      setLoginName("")
      setPassword("")
      setConfirmPassword("")
  ...
  const onLogout = async (e: React.MouseEvent) => {
    ...
    clearShareVideos(prevUserId)
    clearShareVideos()
    setCachedUserId(null)
    setMe(null)
    dispatchAuthChanged({ loggedIn: false, userId: null })
    toast.success("已退出登录")
    setLoggingOut(false)
  }
      ...
      {openMenu && (
        <div className="mt-2 rounded-xl border border-sidebar-border bg-card p-2 soft-shadow">
          <button
            type="button"
            disabled={loggingOut}
            onClick={(e) => void onLogout(e)}
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-foreground transition-colors hover:bg-accent disabled:opacity-60"
          >
