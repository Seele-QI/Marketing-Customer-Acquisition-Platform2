...
def _ensure_db_schema() -> None:
    """幂等初始化 users / sessions / credit_ledger 等表（首次开发必跑）。"""
...
async def share_redirect(token: str):
    ...
    video_url_raw = str(data.get("videoUrl", "") or "").strip()
    video_url_html = html.escape(video_url_raw)
    video_block = ""
    if video_url_raw:
        video_block = f"""
          <div style="margin-top:4px;">
            <div style="font-size:13px;font-weight:600;color:#6b7280;margin-bottom:8px;">视频素材</div>
            <video src="{video_url_html}" controls playsinline style="width:100%;max-height:360px;border-radius:12px;background:#000;" preload="metadata"></video>
            <div style="margin-top:10px;display:flex;flex-wrap:wrap;gap:10px;">
              <a href="{video_url_html}" download style="text-decoration:none;border:1px solid #e5e7eb;background:#ffffff;color:#111827;padding:10px 14px;border-radius:12px;font-size:14px;font-weight:600;display:inline-flex;align-items:center;justify-content:center;">
                下载视频
              </a>
            </div>
          </div>"""
...
<html lang="zh-CN">
  ...
  <body style="margin:0;background:#fafafa;color:#111827;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;">
    ...
    headers = {
        "Content-Security-Policy": "default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src data: https:; media-src https: http: blob:; base-uri 'none'; frame-ancestors 'none'",
...
def _clear_session_cookie(response: Response) -> None:
    """清除 session cookie；属性须与 set_cookie 一致，否则浏览器删不掉 HttpOnly cookie。"""
    response.set_cookie(
        key=SESSION_COOKIE,
        value="",
        max_age=0,
...
_SUPPORTED_ACCOUNT_PLATFORMS = ("douyin", "shipinhao", "xiaohongshu", "kuaishou")
...
# ════════════════════════════════════════════════════════════════════════
#  平台连接器 API（对齐 bony-agent：浏览器登录 + 手动 Cookie）
# ════════════════════════════════════════════════════════════════════════

from lib.connector_service import (
    disconnect_platform as connector_disconnect,
    get_all_platforms,
    save_cookie_payload,
    save_cookie_string,
...
from lib.qr_login_sessions import qr_login_sessions
...
async def connectors_platforms(request: Request):
    user = require_user(request)
    return {"platforms": get_all_platforms(user.id)}


@app.post("/api/connectors/connect")
async def connectors_connect(req: ConnectPlatformRequest, request: Request):
    user = require_user(request)
    platform = req.platform.strip().lower()
    try:
        if req.cookieJson.strip():
            save_cookie_payload(user.id, platform, req.cookieJson.strip())
        elif req.credentials:
            save_platform_credentials(user.id, platform, req.credentials)
        else:
            raise HTTPException(status_code=400, detail="请提供 credentials 或 cookieJson")
        status = next((p for p in get_all_platforms(user.id) if p["platform_id"] == platform), None)
...
async def connectors_disconnect(platform_id: str, request: Request):
    user = require_user(request)
    ok = connector_disconnect(user.id, platform_id)
    return {"success": ok}


@app.post("/api/connectors/browser/start")
async def connectors_browser_start(req: BrowserLoginRequest, request: Request):
    user = require_user(request)
    result = await interactive_login_manager.start_interactive_login(req.platform, user_id=user.id)
    if not result.get("success"):
        return {"success": False, "error": result.get("error") or "启动浏览器失败"}
    return result


@app.post("/api/connectors/browser/status")
async def connectors_browser_status(req: BrowserStatusRequest, request: Request):
    user = require_user(request)
    result = await interactive_login_manager.check_login_status(req.session_id)
    if result.get("status") == "success":
        platform = result.get("platform") or req.platform
        try:
            if result.get("cookie_json"):
                save_cookie_payload(user.id, platform, result["cookie_json"])
            else:
                save_cookie_string(user.id, platform, result.get("cookies", ""))
        except Exception as exc:
            return {"status": "error", "error": f"保存凭证失败: {exc}", "platform": platform}
    return result


@app.post("/api/connectors/browser/cancel")
async def connectors_browser_cancel(req: BrowserStatusRequest, request: Request):
    require_user(request)
    return await interactive_login_manager.cancel_login(req.session_id)
...
@app.post("/api/connectors/qrcode/start")
async def connectors_qrcode_start(req: QrLoginStartRequest, request: Request):
    require_user(request)
    platform = req.platform.strip().lower()
    if platform == "douyin":
        result = await qr_login_sessions.start_douyin()
    else:
        return {"success": False, "error": f"暂不支持 {platform} 扫码登录，请使用浏览器登录或手动 Cookie"}
    if not result.get("success"):
        return {"success": False, "error": result.get("error") or "生成二维码失败，请稍后重试或使用手动 Cookie"}
    return result


@app.post("/api/connectors/qrcode/poll")
async def connectors_qrcode_poll(req: QrLoginPollRequest, request: Request):
    user = require_user(request)
    result = await qr_login_sessions.poll(req.session_id)
    if result.get("status") == "success":
        platform = result.get("platform", "douyin")
        try:
            save_cookie_payload(user.id, platform, result["cookie_json"])
        except Exception as exc:
            return {"status": "error", "error": f"保存凭证失败: {exc}"}
    return result


@app.post("/api/connectors/qrcode/cancel")
async def connectors_qrcode_cancel(req: QrLoginPollRequest, request: Request):
    require_user(request)
    return await qr_login_sessions.cancel(req.session_id)


# ════════════════════════════════════════════════════════════════════════
#  全自动发布 API（Playwright 浏览器自动化）
# ════════════════════════════════════════════════════════════════════════

from lib.publisher.manager import list_bound_platforms, publish_to_all, publish_to_platform
...
def _compose_publish_description(description: str, tags: list[str]) -> str:
    body = (description or "").strip()
    tag_line = " ".join(f"#{t.lstrip('#')}" for t in tags if str(t).strip())
    if tag_line and body:
        return f"{body}\n\n{tag_line}"
    return body or tag_line


@app.get("/api/publish/accounts")
async def publish_accounts(request: Request):
    """已绑定、可用于自动发布的平台账号"""
    user = require_user(request)
    _init_accounts_db()
    return {"accounts": list_bound_platforms(user.id)}


@app.post("/api/publish")
async def publish_single(req: PublishRequest, request: Request):
    """发布到单个平台（全自动）"""
    user = require_user(request)
    _init_accounts_db()
    ...
    if not req.title.strip():
        raise HTTPException(status_code=400, detail="title 不能为空")
    desc = _compose_publish_description(req.description, req.tags)
...
async def publish_all_platforms(req: PublishAllRequest, request: Request):
    """批量发布到多个已绑定平台"""
    user = require_user(request)
    _init_accounts_db()
    if not req.videoUrl.strip():
    ...
    return {"results": results, "success_count": ok, "total": len(results)}
