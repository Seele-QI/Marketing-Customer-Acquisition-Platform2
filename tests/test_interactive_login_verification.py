...
from lib.playwright_env import browser_launch_mode
...
class GeoPlatformLoginTests(unittest.IsolatedAsyncioTestCase):
    def test_toutiao_uses_recoverable_native_cdp_browser(self) -> None:
        self.assertEqual(browser_launch_mode("toutiao"), "native_cdp")

    def test_cdp_session_record_contains_no_cookie_or_token(self) -> None:
        manager = InteractiveBrowserLogin()
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("lib.interactive_login.LOGIN_SESSION_DIR", Path(temp_dir)):
                manager._persist_session_record(
                    "toutiao_session",
                    {
                        "user_id": 4,
                        "platform": "toutiao",
                        "cdp_endpoint": "http://127.0.0.1:19333",
                        "start_time": 123.0,
                        "cookies": "must-not-be-written",
                    },
                )
                payload = json.loads(
                    (Path(temp_dir) / "toutiao_session.json").read_text(encoding="utf-8")
                )
        self.assertEqual(
            set(payload),
            {"session_id", "user_id", "platform", "cdp_endpoint", "start_time"},
        )
