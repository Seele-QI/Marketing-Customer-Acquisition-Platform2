...
def test_legacy_task_directory_is_recovered_after_process_restart(tmp_path):
    from routes import dh_video_economy_routes as routes_mod

    task_id = "dhe_restart_recovery"
    work_dir = tmp_path / task_id / "work"
    segments_dir = work_dir / "segments"
    segments_dir.mkdir(parents=True)
    (work_dir / "avatar.png").write_bytes(b"image")
    (work_dir / "reference.mp3").write_bytes(b"audio")
    (work_dir / "clone_audio.mp3").write_bytes(b"clone")
    (segments_dir / "segment_000.wav").write_bytes(b"a")
    (segments_dir / "segment_001.wav").write_bytes(b"b")

    routes_mod._tasks.pop(task_id, None)
    recovered = routes_mod._recover_task(task_id, user_id=77, root=tmp_path)

    assert recovered is not None
    assert recovered["user_id"] == 77
    assert recovered["status"] == "partial_failed"
    assert recovered["segment_count"] == 2
    assert [item["status"] for item in recovered["segments"]] == ["timeout", "timeout"]
    assert (tmp_path / task_id / "task_state.json").is_file()
