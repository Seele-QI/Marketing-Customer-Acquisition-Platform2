...
import { DISTRIBUTION_ACCOUNTS_CHANGED_EVENT } from "@/lib/distribution/platforms"
import { toast } from "@/hooks/use-toast"
import { getFastapiBase } from "@/lib/fastapi-base"
import { SHARE_VIDEOS_KEY, type ShareVideo } from "@/lib/video/types"
...
export function ShareDistribute({ onNavigateToBinding }: { onNavigateToBinding?: () => void } = {}) {
  ...
  React.useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === SHARE_VIDEOS_KEY || e.key === null) refreshVideos()
    }
    window.addEventListener("storage", onStorage)
    return () => {
      window.removeEventListener("storage", onStorage)
    }
  }, [syncAuth, refreshVideos])

  React.useEffect(() => {
    void loadBoundAccounts()
    const onFocus = () => void loadBoundAccounts()
    window.addEventListener("focus", onFocus)
    window.addEventListener(DISTRIBUTION_ACCOUNTS_CHANGED_EVENT, onFocus)
    return () => {
      window.removeEventListener("focus", onFocus)
      window.removeEventListener(DISTRIBUTION_ACCOUNTS_CHANGED_EVENT, onFocus)
    }
  }, [loadBoundAccounts])
  ...
  React.useEffect(() => {
    const seed = (selectedVideo?.title || "").trim().replace(/[._-]+/g, " ").slice(0, 24)
    if (!seed) {
      setTitleSuggestions([])
      return
    }
    setTitleSuggestions([
      ...
      `${seed}，这几个重点值得看`,
      `真实场景下的${seed}`,
    ].map((item) => item.slice(0, DOUYIN_TITLE_MAX)))
  ...
  const handleAiFillCopy = async () => {
    ...
    try {
      ...
      if (Array.isArray(data.titleCandidates) && data.titleCandidates.length) {
        setTitleSuggestions(data.titleCandidates.slice(0, 3))
      }
  ...
  const handleRemoveVideo = (video: ShareVideo, e: React.MouseEvent) => {
    e.stopPropagation()
    const next = removeShareVideo(video.id)
    setVideos(next)
  ...
  const handleImportFromHistory = () => {
    ...
    const entry: ShareVideo = {
      ...
      url: videoUrl,
      thumbnail: latest.coverThumbnail || latest.coverUrl,
  ...
  const handleLocalVideoUpload = async (file: File) => {
    ...
    try {
      ...
      if (!data.file_url) throw new Error("上传成功，但服务未返回视频地址")
      ...
      <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-10">
        <header className="mb-8">
          ...
          <h1 className="text-[28px] font-bold leading-tight tracking-tight text-slate-900 sm:text-[34px] dark:text-slate-50">
            视频<span className="text-rose-500 dark:text-rose-400">一键分发</span>
          </h1>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                  ...
                  return (
                    ...
                    >
                      <div className="h-14 w-20 shrink-0 overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-800">
                        <video
                          src={`${resolveMediaUrl(video.url)}#t=0.1`}
                          poster={thumb || undefined}
                          preload="metadata"
                          muted
                          playsInline
                          aria-label={`${video.title} 视频封面`}
          ...
          <section className="lg:col-span-7 space-y-4">
            <div className="rounded-2xl border border-slate-200/60 bg-white p-5 dark:border-white/10 dark:bg-white/5">
              ...
              <div className="mt-3 rounded-xl border border-rose-100 bg-rose-50/50 px-3 py-3 dark:border-rose-500/20 dark:bg-rose-500/5">
                <div className="mb-2 flex items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => void handleAiFillCopy()}
                    disabled={isAiFilling || !activeVideoUrl}
                    className="text-[11px] font-medium text-rose-500 disabled:opacity-40"
                  >
                    {isAiFilling ? "生成中..." : "换一批"}
                  </button>
                </div>
                {titleSuggestions.length ? (
                  <div className="flex flex-wrap gap-2">
                    {titleSuggestions.map((suggestion) => (
                      <button
                        key={suggestion}
                        type="button"
                        onClick={() => setTitle(suggestion.slice(0, DOUYIN_TITLE_MAX))}
                        className="rounded-lg border border-white bg-white px-2.5 py-1.5 text-left text-[11px] text-slate-600 shadow-sm hover:border-rose-200 hover:text-rose-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300"
                      >
                        {suggestion}
                      </button>
                    ))}
                  ...
                  <p className="text-[11px] text-slate-400">选择视频后自动生成标题候选</p>
                )}
              </div>

              <div className="mt-3 flex flex-wrap items-center gap-2">
                <span className="text-[11px] text-slate-400">推荐话题</span>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-slate-50 p-1.5 dark:bg-white/5">
                <button type="button" onClick={() => setSubmitMode("manual_confirm")} className={cn("rounded-xl px-3 py-2.5 text-left transition", submitMode === "manual_confirm" ? "bg-white shadow-sm ring-1 ring-rose-200 dark:bg-slate-800" : "text-slate-500")}>
                  <span className="block text-xs font-semibold text-slate-800 dark:text-slate-100">人工确认（推荐）</span>
                  <span className="mt-0.5 block text-[10px] text-slate-500">自动填充，您检查后点击发布</span>
                </button>
                <button type="button" onClick={() => setSubmitMode("auto_submit")} className={cn("rounded-xl px-3 py-2.5 text-left transition", submitMode === "auto_submit" ? "bg-white shadow-sm ring-1 ring-rose-200 dark:bg-slate-800" : "text-slate-500")}>
                  <span className="block text-xs font-semibold text-slate-800 dark:text-slate-100">自动提交</span>
                  <span className="mt-0.5 block text-[10px] text-slate-500">填充后由程序点击最终发布</span>
                </button>
              </div>
              ...
              >
                {isPublishing ? (
                  ...
                  <>
                    ...
                    {submitMode === "manual_confirm" ? "启动半自动分发" : "启动自动分发"}
                  </>
                )}
              </button>
              {isPublishing && publishingPlatform ? (
                <p className="mt-2 text-center text-[12px] text-slate-600 dark:text-slate-400">
                  一次只打开一个 Chrome 窗口；{submitMode === "manual_confirm" ? "系统填好内容后请检查并点击发布，检测成功后进入下一平台。" : "系统将尝试自动提交，遇到验证时请人工接管。"}
                </p>
              ) : null}
