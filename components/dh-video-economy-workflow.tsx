...
import { resolveMediaDownloadUrl, resolveMediaUrl } from "@/lib/video/utils"
...
export default function DhVideoEconomyWorkflow({ initialScript = "" }: { initialScript?: string }) {
  ...
  const videoDownloadUrl = videoUrl
    ? resolveMediaDownloadUrl(videoUrl, `digital-human-economy-${runtimeTask?.taskId || "video"}.mp4`)
    : ""
  const coverDownloadUrl = coverUrl
    ? resolveMediaDownloadUrl(coverUrl, `digital-human-economy-${runtimeTask?.taskId || "cover"}-cover.png`)
    : ""
        ...
        <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          ...
          <section className="space-y-5 rounded-3xl border bg-background p-5 shadow-sm">
            ...
            {!runtimeTask ? <div className="flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed text-center text-muted-foreground"><Loader2 className="mb-3 h-8 w-8 opacity-30" /><p className="text-sm">提交后在这里查看音色克隆、切段、并发生成与拼接进度</p></div> : <>
              ...
              {videoUrl ? <div className="space-y-3"><video controls className="max-h-[520px] w-full rounded-2xl bg-black" src={resolveMediaUrl(videoUrl)} /><div className="flex flex-wrap gap-2"><Button asChild><a href={videoDownloadUrl} download><Download />下载完整视频</a></Button>{coverUrl ? <Button asChild variant="outline"><a href={coverDownloadUrl} download><Download />下载封面</a></Button> : null}</div></div> : null}
